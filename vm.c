...
double popf() {
  if (SP == stack-1) err("trying to pop form empty stack");
  return SP--->f;
}
...
void vmInit(){
	...
	Symbol *fd = addExtFn("put_d", put_d, (Type){ TB_VOID, NULL, -1 });
 	addFnParam(fd, "d", (Type){ TB_DOUBLE, NULL, -1 });
	}
	...
	for(;;){
		...
		switch(IP->op){
			...
			case OP_LESS_D: {
				...
				fBefore=popf();
				pushi(fBefore < fTop);
				printf("LESS.d\t// %g < %g -> %d", fBefore, fTop, fBefore < fTop);
				IP = IP->next;
				break;
				}
